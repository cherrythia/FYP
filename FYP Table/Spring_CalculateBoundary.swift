...
public func Spring_CalculateBoundary (_ f:inout [Float], s:[Float], n:NSInteger) -> (forceBound1:Float, forceBound2: Float, displacement:[Float]) {
    ...
    var displacement = [Float] (repeating: 0.0, count: n+1)
...
//Arrangement of stiffness matrices
    ...
    print("global matrix multiply by displacement")
    for y in 0..<(n+1) {
        for x in 0..<(n+2) {
    ...
    for y in 0...n {
        if(y==0||y==n) { displacement[y] = 0.0 }
            ...
            print(String(format: "%.5f", displacement[y]))
        }
    }
    ...
    print("Force at left wall\t" + String(format: "%.3f", forceBound1))
